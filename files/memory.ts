 ...
 *   SOUL.md (identity) and memories.txt (memory) for context, and notifies the
 *   user that memory is loaded. No write.
 ...
 * during the session by the agent, per ~/.pi/agent/AGENTS.md. This hook
...
export default function (pi: ExtensionAPI) {
  pi.on("session_start", async (event, ctx) => {
    ...
    if (ctx.hasUI) {
      ctx.ui.notify("memories txt loaded", "info");
    }
    ...
    if (event.reason === "startup" || event.reason === "resume" || event.reason === "new") {
      pi.sendMessage(
        {
          customType: "harobe.memory.reminder",
          content:
            "[harobe:memory] Session started. Read ~/.pi/agent/SOUL.md (identity) and ~/.pi/agent/memories.txt (append-only memory log) for context. If something worth keeping happens this session (decisions, new setup, the users preferences, resolved TODOs), append a JSON line to memories.txt — never edit old lines.",
          display: false,
        },
